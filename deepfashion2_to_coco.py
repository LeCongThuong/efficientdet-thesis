...
def convert_format(anno_dir, imag_dir):
    ...
    num_images = len([image for image in os.listdir(imag_dir)])
    num_annos = len([anno for anno in os.listdir(anno_dir)])
    if num_images != num_annos:
        raise Exception("The number of images is NOT EQUAL to the number of annotations")

    sub_index = 0 # the index of ground truth instance

    for num in range(1, num_images+1):
        json_name = anno_dir + '/' + str(num).zfill(6)+'.json'
        image_name = imag_dir + '/' + str(num).zfill(6)+'.jpg'
...
def main():
    parse = argparse.ArgumentParser(description="DeepFashion2-to-COCO converter")
    parse.add_argument("-i",
                       "--inputDir",
                       help="Path to folder where image and anno folder are stored",
                       type=str)
    parse.add_argument("-o",
                       "--outputFile",
                       help="Name of output .json file(include path)",
                       type=str)

    args = parse.parse_args()
    if args.inputDir is None:
        args.inputDir = '/home/love_you/Documents/Study/Thesis/clothes-detection/workspace/training_demo/images/train/'
    if args.outputFile is None:
        args.outputFile = args.inputDir + "/retrieval_train_shop_information.json"

    anno_dir = os.path.join(args.inputDir, 'annos')
    image_dir = os.path.join(args.inputDir, 'images')
    dataset = convert_format(anno_dir, image_dir)
